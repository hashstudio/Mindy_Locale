...
class DateTimeParser
{
    ...
    public static function parse($value, $pattern = 'MM/dd/yyyy', $defaults = [])
    {
        ...
        $i = 0;
        $n = self::$_mbstringAvailable ? mb_strlen($value, Mindy::app()->charset) : strlen($value);
    ...
    private static function tokenize($pattern)
    {
        ...
        for ($start = 0, $i = 1; $i < $n; ++$i) {
            $c = self::$_mbstringAvailable ? mb_substr($pattern, $i, 1, Mindy::app()->charset) : substr($pattern, $i, 1);
            if ($c !== $c0) {
                $tokens[] = self::$_mbstringAvailable ? mb_substr($pattern, $start, $i - $start, Mindy::app()->charset) : substr($pattern, $start, $i - $start);
                $c0 = $c;
                $start = $i;
            }
        }
        $tokens[] = self::$_mbstringAvailable ? mb_substr($pattern, $start, $n - $start, Mindy::app()->charset) : substr($pattern, $start, $n - $start);
        return $tokens;
    }
    ...
    {
        $v = strtolower(self::$_mbstringAvailable ? mb_substr($value, $offset, 2, Mindy::app()->charset) : substr($value, $offset, 2));
